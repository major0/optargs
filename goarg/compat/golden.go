...
// GoldenPath returns the path for a golden file (for external use in tests).
func GoldenPath(name string) string {
	return filepath.Join("testdata", name+".golden.json")
}

// GoldenExists returns true if the named golden file exists.
func GoldenExists(name string) bool {
	_, err := os.Stat(GoldenPath(name))
	return err == nil
}

// FormatGoldenName builds the golden file name from scenario and kind.
func FormatGoldenName(scenario, kind string) string {
	return fmt.Sprintf("%s.%s", scenario, kind)
}
